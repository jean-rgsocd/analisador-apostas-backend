...
# Versão 27.1 - Análise completa para todos os esportes (Revisado e Corrigido)
...
app = FastAPI(title="Sports Betting Analyzer - Multi-Sport Full", version="27.1")
origins = ["*"]
...
# --- ENDPOINTS DE PAÍSES, LIGAS E JOGOS (sem alterações) ---
@app.get("/paises")
...
async def get_games_by_filter(sport: str, league_id: int):
    ...
    if sport == 'football': querystring["season"] = str(datetime.now().year)
    if sport != 'formula-1':
...
async def analyze_nba_pre_game(game_id: int, headers: dict) -> List[TipInfo]:
    # A lógica detalhada da NBA que criamos anteriormente iria aqui.
    # Por enquanto, usaremos a genérica para consistência.
    return await analyze_generic_pre_game(game_id, "nba", headers)

async def analyze_generic_pre_game(game_id: int, sport: str, headers: dict) -> List[TipInfo]:
    tips = []
    base_url = f"https://{SPORTS_MAP[sport]['host']}"
    url = f"{base_url}/games?id={game_id}" # Padrão para a maioria
    if sport == 'formula-1':
        url = f"{base_url}/races?id={game_id}"
    ...
    if game_data:
        item = game_data[0]
        if sport in ['basketball', 'nfl', 'baseball', 'nba', 'rugby', 'volleyball', 'hockey', 'handball']:
            ...
            fighter1 = item.get("fighters", [{}])[0].get("name", "Lutador 1")
...
async def analyze_pre_game_endpoint(game_id: int, sport: str):
    ...
    # A NBA pode ter sua lógica customizada aqui, mas por agora usa a genérica
    # elif sport == "nba":
...
async def analyze_live_game_endpoint(game_id: int, sport: str):
    ...
    # Análise ao vivo para outros esportes é mais complexa e fica como placeholder
