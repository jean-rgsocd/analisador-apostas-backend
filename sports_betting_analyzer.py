...
# Versão 27.0 - Correção final: Adiciona 'season' em todas as buscas de jogos
...
app = FastAPI(title="Sports Betting Analyzer - Multi-Sport Final", version="27.0")
origins = ["*"]
...
# --- ENDPOINTS DE PAÍSES, LIGAS E JOGOS ---
@app.get("/paises")
...
async def analyze_football_pre_game(game_id: int, headers: dict) -> List[TipInfo]:
    tips = []; base_url = "https://v3.football.api-sports.io"
    async with httpx.AsyncClient() as client:
    ...
    if not tips:
        tips.append(TipInfo(market="Análise Conclusiva", suggestion="Equilíbrio", justification="Os dados pré-jogo não apontam um favoritismo claro.", confidence=0))
    return tips
...
async def analyze_football_live_game(game_id: int, headers: dict) -> List[TipInfo]:
    ...
    if not fixture_data or len(stats_data) < 2:
        return [TipInfo(market="Análise Ao Vivo", suggestion="Aguardar", justification="Estatísticas ao vivo ainda não disponíveis.", confidence=0)]
    ...
    if not tips:
        tips.append(TipInfo(market="Análise Ao Vivo", suggestion="Aguardar", justification="Jogo sem oportunidades claras no momento.", confidence=0))
    return tips

async def analyze_nba_pre_game(game_id: int, headers: dict) -> List[TipInfo]:
    tips = []; base_url = "https://v2.nba.api-sports.io"
    async with httpx.AsyncClient() as client:
    ...
    if h2h_data:
        ...
        if home_wins > away_wins and len(h2h_data) > 0 and home_wins / len(h2h_data) >= 0.7:
        ...
        elif away_wins > home_wins and len(h2h_data) > 0 and away_wins / len(h2h_data) >= 0.7:
...
async def analyze_generic_pre_game(game_id: int, sport: str, headers: dict) -> List[TipInfo]:
    ...
    if sport == 'formula-1': url = f"{base_url}/races?id={game_id}"
    
    async with httpx.AsyncClient() as client:
        game_data = await fetch_api_data_async(client, {}, headers, url)

    if game_data:
        item = game_data[0]
        if sport in ['basketball', 'nfl', 'baseball', 'rugby', 'volleyball', 'hockey', 'handball']:
            home = item.get("teams", {}).get("home", {}).get("name", "Casa")
            tips.append(TipInfo(market="Vencedor da Partida", suggestion=f"Analisar {home}", justification="Análise genérica baseada em favoritismo e dados disponíveis.", confidence=60))
        elif sport == 'formula-1':
            tips.append(TipInfo(market="Vencedor da Corrida", suggestion="Analisar piloto da pole", justification="Análise genérica baseada em posição de largada.", confidence=65))
        elif sport == 'mma':
            fighter1 = item.get("fighters", [{}])[0].get("name", "Lutador 1") if item.get("fighters") else "Lutador 1"
            tips.append(TipInfo(market="Vencedor da Luta", suggestion=f"Analisar {fighter1}", justification="Análise genérica baseada no card.", confidence=60))

    if not tips:
        tips.append(TipInfo(market="Análise", suggestion="Equilíbrio", justification="Dados insuficientes para uma análise detalhada.", confidence=50))
    return tips
...
async def analyze_pre_game_endpoint(game_id: int, sport: str):
    ...
    if sport == "football":
        ...
        return await analyze_generic_pre_game(game_id, sport, headers)
...
async def analyze_live_game_endpoint(game_id: int, sport: str):
    ...
    return [TipInfo(market="Análise Ao Vivo", suggestion="Aguardando dados", justification=f"Análise ao vivo para {sport.capitalize()} não implementada.", confidence=0)]
