...
# VERSÃO FINAL E DEFINITIVA - À PROVA DE BALAS
...
@app.get("/ligas/football/{country_identifier}")
def get_leagues_by_country(country_identifier: str) -> List[Dict[str, Any]]:
    # --- LÓGICA À PROVA DE BALAS ---
    # Verifica se o identificador é um código (2 letras) ou um nome.
    if len(country_identifier) == 2:
        params = {"code": country_identifier.upper()}
    else:
        params = {"country": country_identifier.capitalize()}
    
    params["season"] = get_season_for_sport("football")
    
    cache_key = f"leagues_football_{country_identifier.lower()}"
    if cache_key in cache and datetime.now() < cache[cache_key]["expiry"]: return cache[cache_key]["data"]
...
def get_pre_game_analysis(game_id: int, sport: str):
    ...
    if sport == 'football':
        ...
        fixture_params = {"id": game_id}
        fixture_data = call_any_api(fixture_url, fixture_params)
        if fixture_data:
    ...
    if not data or not data[0].get("bookmakers"): return [{"market": "Indisponível", "suggestion": "N/A", "justification": "As odds para este jogo ainda não foram publicadas.", "confidence": 0}]
    ...
    if not analysis_tips: return [{"market": "Análise Padrão", "suggestion": "N/A", "justification": "Não foram encontrados mercados com alta probabilidade para análise automática.", "confidence": 0}]
