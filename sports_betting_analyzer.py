...
def get_season_for_sport(sport: str) -> str:
    """Retorna a temporada atual para cada esporte"""
    now = datetime.now()
    year = now.year
    if sport == "basketball":
        # NBA temporada atravessa 2 anos
...
def get_football_countries() -> List[Dict[str, str]]:
    """Lista de países disponíveis no futebol"""
...
def get_leagues_by_country(country_code: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        data = response.json().get("response", [])
        leagues = [
            {"id": l["league"]["id"], "name": l["league"]["name"]}
            for l in data if l.get("league")
        ]
...
def get_basketball_leagues() -> List[Dict[str, Any]]:
    """Ligas fixas da NBA"""
    return [{"id": "standard", "name": "NBA Regular Season"}]


@app.get("/ligas/american-football")
def get_nfl_leagues() -> List[Dict[str, Any]]:
    """Ligas fixas da NFL"""
...
def get_games_by_league(sport: str, league_id: str) -> List[Dict[str, Any]]:
    """Próximos jogos de acordo com o esporte e liga"""
...
def get_pre_game_analysis(game_id: int, sport: str):
    """Análises pré-jogo baseadas em odds"""
    ...
    if not data or not data[0].get("bookmakers"):
        return [{
            "market": "Indisponível",
            "suggestion": "N/A",
            "justification": "As odds para este jogo ainda não foram publicadas.",
            "confidence": 0,
        }]
    ...
    if winner_bet and len(winner_bet.get("values", [])) >= 2:
        ...
        if fav_odd < 1.7:
            analysis_tips.append({
                ...
                "justification": f"O mercado aponta um favoritismo claro para o {fav_team}, com odds de {fav_odd}.",
                "confidence": 85,
            })
    ...
    if total_bet and len(total_bet.get("values", [])) > 0:
        ...
        analysis_tips.append({
            ...
            "justification": f"A linha principal está em {line}. Times ofensivos tendem a superar essa marca.",
            "confidence": 70,
        })

    if not analysis_tips:
        return [{
            "market": "Análise Padrão",
            "suggestion": "N/A",
            "justification": "Não foram encontrados mercados com alta probabilidade para análise automática.",
            "confidence": 0,
        }]
